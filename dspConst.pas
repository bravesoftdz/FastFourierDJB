...
type
  ...
  TDCBitRate = (br8BitInteger, br16BitInteger, br24BitInteger, br32BitInteger,
    br32BitFloat);

  { Specifys the WindowMode used by GetWindowingValue. }
  TWindowMode = (wmRectangular, wmHanning, wmBartlett, wmHamming, wmGaussian,
    wmBlackman, wmBlackmanHarris);

  { Used by TDCFFT to setup the FFT Size. }
  TDCFFTSize = (fts2, fts4, fts8, fts16, fts32, fts64, fts128, fts256, fts512,
    fts1024, fts2048, fts4096, fts8192, fts16384);
  ...
  TPass = array [0 .. 2] of array [0 .. MaxChannels - 1] of Float;
  { Used internal by the Flanger Component to temporary store previous processed Samples. }
  TFlangerPrevOut = array [0 .. MaxChannels - 1] of array [0 .. 100000]
    of Float;
  ...
  TVisualBuffer = array [0 .. MaxChannels - 1] of array
    [0 .. MaxVisualSamples - 1] of integer;
  { Used internal by the TDCWaveform and TDCSpectrum Class to Send an Event that the Visual Buffer has been Processed. }
  TDCVisualNotifyEvent = procedure(Sender: TObject; Data: PVisualBuffer;
    MinY, MaxY, NumSamples, Channels: integer) of object;
